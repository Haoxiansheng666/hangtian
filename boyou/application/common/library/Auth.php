...
class Auth
{
    ...
    protected $config = [];
    protected $options = [];
    protected $allowFields = ['id', 'username', 'nickname', 'mobile', 'avatar', 'is_in','type','qrcode'];
     ...
     * @return boolean
     */
    public function register($username, $password = '123456', $email = '', $mobile = '', $extend = [])
    {
        // 检测用户名、昵称、邮箱、手机号是否存在
        ...
        try {
            ...
            $qrcode = \request()->domain(). "/qrcode/build?text={$user->id}&label=&logo=0&labelalignment=center&foreground=%23008000&background=%23ffffff&size=300&padding=10&logosize=50&labelfontsize=14&errorcorrection=medium";
            $this->_user = User::get($user->id);

            User::where('id',$user->id)->update(['qrcode'=>$qrcode]);
            //设置Token
     ...
     */
    public function mobile_login($mobile)
    {
        $user = User::get(['mobile'=>$mobile]);
        if($user){
            return $this->direct($user->id);
        }
        return $this->register($mobile,Random::alnum(6),'',$mobile);
    }
    ...
    public function getUserinfo()
    {
        ...
        $company = Company::where('user_id',$userinfo['id'])->find();
        //已提交 审核中
        if($company && $company['status'] == 0){
            $userinfo['is_in'] = 2;
        }elseif ($company && $company['status'] == 2){
            //已提交 审核拒绝  不可进 可选择身份
            $userinfo['is_in'] = 0;
        }
        return $userinfo;
