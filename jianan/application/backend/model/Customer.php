...
{   
    ...
    protected $renTime = 'ren_time';
    protected $updateTime = false;

    // 类型转换
    protected $type = array(
        'create_time' => 'timestamp:Y-m-d H:i:s',
        'ren_time' => 'timestamp:Y-m-d H:i:s',
    ...
    public function getGenderAttr($value){
        return !empty($value) ? $value == 1 ? '男' : '女' : '未知';
    }
    public function getFromTextAttr($value,$data){
        return $data['from'] == 1 ? '个人' : '企业';
    }
    public function getFollowStatusTextAttr($value,$data){
        ...
        return !empty($status[$data['follow_status']]) ? $status[$data['follow_status']] : '状态未知';
    }
    ...
    public function get_join_list($param,$where=[]){
        $res =  $this
            ->with('admin,authGroup,cate,profession,professionTop')
            ->where($where)
            ->order('id DESC')
    ...
    public function get_joinList($param,$where=[],$where1=[]){
        $res =  $this
            ->with('admin,authGroup,cate,profession,professionTop')
            ->where($where)
            ->where($where1)
    ...
    public function get_down_list($param,$where='1=1'){
        ...
        foreach ($ress as $k => $v) {
            ...
            $rs[$k]['create_time'] = change_date($v['create_time'],2);
