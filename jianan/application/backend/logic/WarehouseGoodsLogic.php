        ...
        for ($row = 1;$row <= $highestRow;$row++)
        {
            ...
            if($row==1){
                ...
                if($fromlist){
                    ...
                    $ffe=implode(',', $fe);
                    if(!in_array($data[$row]['from'], $fe) && !in_array($data[$row]['from'],$fe_ids)){
                ...
                if($catelist){
                    ...
                    $ff=implode(',', $ftype);
                    if(!in_array($data[$row]['cate_id'], $ftype) && !empty($data[$row]['cate_id'])){
                        $msg='第'.$row."行数据，客户分类:".$data[$row]['cate_id']."，不正确，正确的类型为：".$ff;
                        $status=-1;
                        break;
                    }
                    if (!empty($data[$row]['cate_id'])){
                        //反转充值类型
                        $dd=array_flip($ftype);
