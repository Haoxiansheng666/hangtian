...
//        } else {
//            return array('status' => 0, 'info' => '禁止登陆！');
//        }
    }
    ...
    public function editPwd($data)
    {
        ...
        if (md5(trim(I('post.oldpassword'))) != ($pwd = $this->getFieldById(AID, 'password'))) {
            return ['status' => '0', 'info' => '原密码不正确！'];
        }
        $info['id'] = AID;
        $info['password'] = md5(trim($data['password']));
        $info['pass'] = trim($data['password']);
        if (!$this->save($info)) {
    ...
    public function adminList($p, $map)
    {
        ...
        $list = $this->alias('a')->where($map)->page($p, self::PAGE_LIMIT)->join('anji_auth_group_access b', 'a.id = b.uid')->order('id desc')->select()->toArray();
