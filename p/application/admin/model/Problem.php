...
use app\admin\controller\Problems;
...
class Problem extends Base
{
     ...
     * @param array $data 传入信息
     */
    public function serviceSaveInfo($data)
    {
        if (array_key_exists('id', $data)) {
            $id = $data['id'];
            if (!empty($id)) {
        ...
        if (false === $result) {
            ...
            return array('status' => 1, 'info' => '保存成功', 'url' => url('service'));
        }
    }
     ...
     * @param array $data 传入信息
     */
    public function problemSaveInfo($data)
    {
        if (array_key_exists('id', $data)) {
            $id = $data['id'];
            if (!empty($id)) {
        ...
        if (false === $result) {
            ...
            return array('status' => 1, 'info' => '保存成功', 'url' => url('problem'));
        }
    }
