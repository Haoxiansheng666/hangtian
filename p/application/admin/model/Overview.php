...
class Overview extends Base
{
     ...
     * @param array $data 传入信息
     */
    public function advaSaveInfo($data)
    {
        if (array_key_exists('id', $data)) {
            $id = $data['id'];
            if (!empty($id)) {
        ...
        if (false === $result) {
            ...
            return array('status' => 1, 'info' => '保存成功', 'url' => url('adva'));
        }
    }
     ...
     * @param array $data 传入信息
     */
    public function ldeaSaveInfo($data)
    {
        if (array_key_exists('id', $data)) {
            $id = $data['id'];
            if (!empty($id)) {
        ...
        if (false === $result) {
            ...
            return array('status' => 1, 'info' => '保存成功', 'url' => url('ldea'));
        }
    }
