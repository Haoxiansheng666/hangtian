...
class Picture extends Base
{
     ...
     * @return array      返回列表
     */
    public function infoList($p, $where)
    {

        $request = Request::instance();
        $list = $this->where($where)->order('id desc')->page($p, self::PAGE_LIMIT)->select()->toArray();
        foreach ($list as $k => $v) {
            $list[$k]['child'] = db('notice')->where('tid', $v['id'])->select();
            $list[$k]['status'] = $v['type'] == 1 ? '公司照片' : ($v['type'] == 2 ? '产品照片' : '产品视频');
        }
    ...
    public function saveVideo($data)
    {
        ...
        if (false === $result) {
            ...
            return array('status' => 1, 'info' => '保存成功', 'url' => url('video'));
        }
    }
    /**
     * 改变显示状态 is_show
